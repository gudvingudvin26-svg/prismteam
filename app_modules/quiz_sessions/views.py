...
class QuizSessionViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        """Динамическое определение прав: публичный доступ для игры, аутентификация для управления."""
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({'id': session.id, 'code': code, 'quiz': quiz.id, 'status': session.status}, status=status.HTTP_201_CREATED)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)

    @action(detail=False, methods=['post'], permission_classes=[permissions.AllowAny()], url_path='join')
    def join(self, request):
        """POST join: присоединение участника к сессии по коду с созданием персональной сессии."""
        ...
        try:
            ...
            if existing_player:
                return Response({'session_id': existing_player.id, 'code': existing_player.code, 'quiz_title': existing_player.quiz.title}, status=status.HTTP_200_OK)
            player_session = QuizSession.objects.create(quiz=master_session.quiz, code=master_session.code, participant_name=nickname, status='waiting', started_at=timezone.now())
            print(f"Player session created: id={player_session.id}, name={player_session.participant_name}")
            return Response({'session_id': player_session.id, 'code': player_session.code, 'quiz_title': player_session.quiz.title}, status=status.HTTP_200_OK)
        except Exception as e:
    ...
    def end(self, request, pk=None):
        """POST end: завершение сессии, уведомление наблюдателей и логирование."""
        try:
            session = QuizSession.objects.get(id=pk)
    ...
    def my_result(self, request, pk=None):
        ...
        try:
            ...
            all_sessions = QuizSession.objects.filter(code=session.code, participant_name__isnull=False)
            scores = {}
                ...
                for ans in s_answers:
                    if ans.is_correct:
                        ...
                        if q.question_type == 'multiple':
                            ...
                            points += q_points
                if name == "Организатор" and points == 0 and all_sessions.exclude(participant_name__isnull=True).exists():
                    continue
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        try:
            ...
            print(f"Retrieve session {kwargs.get('pk')}, status: {session.status}, participant: {session.participant_name}")
        except QuizSession.DoesNotExist:
            return Response({'error': 'Session not found'}, status=status.HTTP_404_NOT_FOUND)
