...
class UserRegistration(APIView):
    """Регистрация пользователя через веб-форму с авторизацией по JWT."""
    throttle_classes = [AnonRateThrottle]

    def post(self, request):
        """Обработка POST: валидация, создание пользователя, установка токенов в cookie."""
    ...
    def get(self, request):
        """Обработка GET: редирект для авторизованных или форма регистрации."""
...
class UserLogin(APIView):
    """Аутентификация пользователя по username/email с выдачей JWT-токенов."""
    throttle_classes = [AnonRateThrottle]

    def post(self, request):
        """Обработка POST: проверка учётных данных, вход, установка токенов."""
        ...
        current_user = User.objects.filter(email=identification_parameter).first() if '@' in identification_parameter else User.objects.filter(username=identification_parameter).first()
        if not current_user:
    ...
    def get(self, request):
        """Обработка GET: редирект для авторизованных или форма входа."""
        if request.user.is_authenticated:
            return redirect('/dashboard')
...
class UserLogout(APIView):
    ...
    def post(self, request):
        """POST: logout, удаление cookie, логирование."""
        ...
        for cookie in ['access_token', 'refresh_token', 'csrftoken', 'sessionid']:
...
class UserRegistrationAPI(APIView):
    ...
    def post(self, request):
        ...
        password, again_password = request.data.get('password'), request.data.get('again_password')
        first_name = request.data.get('first_name', '')
        if not username or not email or not password:
            return Response({'error': 'Требуется имя пользователя, email и пароль'}, status=status.HTTP_400_BAD_REQUEST)
        if password != again_password:
            return Response({'error': 'Пароли не совпадают'}, status=status.HTTP_400_BAD_REQUEST)
        if len(password) < 6:
        ...
        current_user = serializer.save()
        current_user.first_name, current_user.set_password(password)
        current_user.save()
        refresh = RefreshToken.for_user(current_user)
        login(request, current_user)
        ...
        for name, token, max_age in [('access_token', str(refresh.access_token), 3600*24), ('refresh_token', str(refresh), 3600*24*7)]:
            response.set_cookie(name, token, httponly=True, secure=True, samesite='None', max_age=max_age, path='/')
        return response
...
class QuestionViewSet(viewsets.ModelViewSet):
    ...
    throttle_classes = [UserRateThrottle]
    def get_queryset(self):
        """QuerySet вопросов с оптимизированными связями."""
        user = self.request.user
        return Question.objects.filter(quiz__created_by=user).select_related('quiz').prefetch_related('answer_options')
...
class AnswerOptionViewSet(viewsets.ModelViewSet):
    ...
    throttle_classes = [UserRateThrottle]
    def get_queryset(self):
        """QuerySet вариантов ответов с оптимизированными связями."""
        user = self.request.user
        return AnswerOption.objects.filter(question__quiz__created_by=user).select_related('question__quiz')