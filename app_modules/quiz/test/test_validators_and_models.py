...
class TestModels(TestCase):
    ...
    def test_question_str_truncation(self):
        long_text = "A" * 100
        q = Question.objects.create(quiz=self.quiz, text=long_text, order=2)
    ...
    def test_question_clean_whitespace_only(self):
        q = Question(quiz=self.quiz, text="   ")
        with self.assertRaises(DjangoValidationError) as ctx:
            q.full_clean()
        self.assertIn("text", ctx.exception.message_dict)

    def test_question_ordering(self):
        q2 = Question.objects.create(quiz=self.quiz, text="Q2", order=0)
        q3 = Question.objects.create(quiz=self.quiz, text="Q3", order=2)
        ordered = list(Question.objects.filter(quiz=self.quiz).values_list("order", flat=True))
...
class TestValidators(TestCase):
    ...
    def test_no_correct_answer(self):
        ...
        with self.assertRaises(DjangoValidationError):
            validate_answer_options_data(opts, use_drf_exception=False)
    ...
    def test_question_with_empty_text(self):
        q = Question.objects.create(quiz=self.quiz, text="")
    ...
    def test_question_with_short_text(self):
        q = Question.objects.create(quiz=self.quiz, text="Hi")
        AnswerOption.objects.create(question=q, text="A", is_correct=True)
        AnswerOption.objects.create(question=q, text="B", is_correct=False)
        with self.assertRaises(DjangoValidationError):
            validate_quiz_integrity(self.quiz, use_drf_exception=False)
    ...
    def test_question_with_less_than_two_answers(self):
        q = Question.objects.create(quiz=self.quiz, text="Valid question?")
        AnswerOption.objects.create(question=q, text="A", is_correct=True)
        with self.assertRaises(DjangoValidationError):
            validate_quiz_integrity(self.quiz, use_drf_exception=False)
