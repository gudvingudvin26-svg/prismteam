"""Сериализаторы для моделей квиз-приложения: User, Question, AnswerOption, Quiz."""
import logging
...
class UserSerializer(serializers.ModelSerializer):
    """Сериализатор пользователя: регистрация и базовые поля."""
    class Meta:
        model = User
...
class AnswerOptionSerializer(serializers.Serializer):
    """Сериализатор варианта ответа: текст и флаг правильности."""
...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    def validate_answer_options(self, value):
        """Валидация списка вариантов ответа через внешний валидатор."""
        validate_answer_options_data(value)
        return value

    def validate(self, data):
        """Комплексная валидация вопроса: текст, варианты, тип вопроса."""
        text = data.get('text')
    ...
    def create(self, validated_data):
        """Создание вопроса с массовым созданием связанных вариантов ответов."""
    ...
    def update(self, instance, validated_data):
        """Обновление вопроса и замена вариантов ответов с логированием."""
        ...
        if options_data is not None:
            ...
            AnswerOption.objects.bulk_create([AnswerOption(question=instance, **opt) for opt in options_data])
        quiz_log.info('Организатор успешно обновил вопрос в квизе')
        return instance


class QuizSerializer(serializers.ModelSerializer):
    """Сериализатор квиза с вложенными вопросами и авто-привязкой создателя."""
    questions = QuestionSerializer(many=True, read_only=True)
    created_by = serializers.HiddenField(default=serializers.CurrentUserDefault())

    class Meta:
        model = Quiz
        fields = ['id', 'title', 'description', 'created_by', 'access_token', 'timer', 'points_per_question', 'questions']
